...
public class FanaticsHome {
  ...
  public Fanatics findById(int id) {
    log.debug("getting Fanatics instance with id: " + id);
    try {
      Fanatics instance = (Fanatics) sessionFactory.getCurrentSession().get("myfan.dao.Fanatics", id);
      if (instance == null) {
  ...
  public List findByExample(Fanatics instance) {
    ...
    try {
      List results = sessionFactory.getCurrentSession().createCriteria("myfan.dao.Fanatics")
