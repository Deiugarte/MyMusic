...
public class MembersHome {
  ...
  public Members findById(java.lang.Integer id) {
    ...
    try {
      Members instance = (Members) sessionFactory.getCurrentSession().get("myfan.dao.temp.Members", id);
      if (instance == null) {
  ...
  public List<Members> findByExample(Members instance) {
    log.debug("finding Members instance by example");
    try {
      List<Members> results = (List<Members>) sessionFactory.getCurrentSession()
          .createCriteria("myfan.dao.temp.Members").add(create(instance)).list();
